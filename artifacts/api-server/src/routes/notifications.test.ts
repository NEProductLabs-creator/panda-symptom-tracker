/**
 * Integration tests for notificationsLimiter — applied to /api/notifications/*.
 *
 * notificationsLimiter config: limit=20, windowMs=60s, standardHeaders="draft-7",
 * keyGenerator = IP + userId (composite) or IP-only (unauthenticated).
 *
 * Tests use POST /api/notifications/unsubscribe (no VAPID dependency) so they
 * stay independent of push key configuration.
 *
...
// ---------------------------------------------------------------------------
// Mock web-push to prevent module-level setVapidDetails side-effects.
// ---------------------------------------------------------------------------

function registerWebPushMock() {
  vi.doMock('web-push', () => ({
    default: {
...
describe('notificationsLimiter — threshold (20 req/min)', () => {
  let app: Application;
...
describe('notificationsLimiter — IP+userId composite key isolation', () => {
  let app: Application;
